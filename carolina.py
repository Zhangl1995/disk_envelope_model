...
#
# ============================================================================================================================
# DEFINITION DUST DENSITY IN PPDSIK MODEL
...
#def getDustDensity(grid=None, ppar=None):
    """
        ...
        """
    print "Using get dust from model_carolina.py"
    ...
    for iz in range(grid.nz):
        for iy in range(grid.ny):
            for ix in range(grid.nx):
                if np.abs(yy[ix,iy]) <= np.abs(thetac): rho_env[ix,iy,iz,0] = ppar['rho0Env'] * 1./(1. + (xx[ix,iy]/ppar['rzero'])**ppar['prhoEnv'])
